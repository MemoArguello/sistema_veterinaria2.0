...
if (empty($_POST["nombre"]) || empty($_POST["especie"]) || empty($_POST["raza"]) || empty($_POST["sexo"])) {
    ...
    $guardar = $conexion->prepare("INSERT INTO mascota (nombre, especie, raza, sexo) VALUES (?, ?, ?, ?)");
    $resultado = $guardar->execute([$nombre, $especie, $raza, $sexo]);
    if ($resultado === TRUE) {
